...
def update_note(noteId: str, payload: schemas.NoteBaseSchema, db: Session = Depends(get_db)):
    ...
    if not db_note: raise HTTPException(status_code = status.HTTP_404_NOT_FOUND, 
                                       detail = f'Nonotewiththisid:{noteId}found')
...
def get_post(noteId: str, db: Session = Depends(get_db)):
    ...
    if not note: 
        raise HTTPException(status_code = status.HTTP_404_NOT_FOUND, detail = f"Nonotewiththisid:{id}found")
